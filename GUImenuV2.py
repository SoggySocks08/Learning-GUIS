...
# Submit button (seperate from menu item buttons)
    ...
    self.submit.grid(row=4,column=0, pady=25)

# End Order Button (Seperate from all other buttons)
    self.endOrderbtn = Button(self.frame1,
                              text="All Done!",
                              width=15,
                              height=2,
                              command=self.endOrder)
    self.endOrderbtn.grid(row=4,column=1, pady=25)
  
  def create_frame1_labels(self):
     self.orderLbl = Label(self.frame1,
                      text=f"Order:{self.order}")
     self.orderLbl.grid(row=0,column=5)
...
# ===================
  ...
  def inv_func(self):
      ...
      for item, count in self.inventory_dict.items():
          print(f"{item.lower()}: {count}")
      print("\nRestocks:")
      for item, restocks in self.restocks_dict.items():
          print(f"{item.lower()}: {restocks}")
